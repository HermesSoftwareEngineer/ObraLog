...
def agent_node(state: State, config: RunnableConfig | None = None) -> dict:
    ...
    cached_system = state.get("system_message")
    cached_count = state.get("_system_msg_human_count")

    if cached_system is None or cached_count != human_count:
        system = _build_system_message(messages, config)
        system_updates = {"system_message": system, "_system_msg_human_count": human_count}
    else:
    ...
    return {"messages": [response], **system_updates}
