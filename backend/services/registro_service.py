"""Serviço de registros de produção — operações do agente IA."""
...
from backend.utils.storage import upload_imagem_registro

logger = logging.getLogger("obralog.services.registro")

_STATUS_ALIASES: dict[str, RegistroStatus] = {
...
def _parse_status(value: str | None) -> RegistroStatus | None:
    if not value:
        return None
    parsed = _STATUS_ALIASES.get(str(value).strip().lower())
    if not parsed:
...
def criar_registro(
    ...
    *,
    ...
    loc = localizacao or {}
    location_type = str(loc.get("tipo") or "estaca").strip().lower()
    start_value = loc.get("valor_inicial", estaca_inicial if estaca_inicial is not None else km_inicial)
    end_value = loc.get("valor_final", estaca_final if estaca_final is not None else km_final)
    detail_value = loc.get("detalhe_texto", local_descritivo)

    if resultado is None and start_value is not None and end_value is not None:
        resultado = float(end_value) - float(start_value)

    parsed_status = _parse_status(status) or RegistroStatus.PENDENTE
    ...
    with SessionLocal() as db:
        ...
        if source_message_id:
            try:
                ...
                raise ValueError("source_message_id inválido.") from exc
        ...
        if resolved_frente_id is not None:
            schema = RegistroSchemaRepository.obter_para_frente(db, resolved_frente_id, tenant_id)
            if schema:
        ...
        metadata: dict = {"tipo": location_type}
        ...
        registro = Repository.registros.criar(
            ...
            lado_pista=parse_lado_pista(lado_pista),
...
def obter_registro(tenant_id: int, actor_user_id: int, actor_level: str, registro_id: int) -> dict:
    ...
    with SessionLocal() as db:
        ...
        data["registrador_nome"] = (
            registro.usuario_registrador.nome
            if getattr(registro, "usuario_registrador", None) else None
        )
    ...
    *,
    ...
    with SessionLocal() as db:
        if actor_level == NivelAcesso.ENCARREGADO.value:
            ...
            resolved = resolve_frente_servico_id(
                db,
                frente_servico_id=frente_servico_id,
                frente_servico_nome=frente_servico_nome,
                tenant_id=tenant_id,
            )
            registros = Repository.registros.listar_por_frente(db, resolved, tenant_id=tenant_id)
        ...
        return [registro_to_dict_with_images(db, item) for item in registros[:limit]]
...
def atualizar_registro(
    ...
    *,
    ...
    if campos_extras_valores and isinstance(campos_extras_valores, dict):
        payload["metadata_json"] = {**payload["metadata_json"], **campos_extras_valores}
    if data:
        payload["data"] = date.fromisoformat(data)
    if start_value is not None and end_value is not None and resultado is None:
        payload["resultado"] = float(end_value) - float(start_value)
    if source_message_id is not None:
        payload["source_message_id"] = (
            None if source_message_id == ""
            else UUID(str(source_message_id))
        )
...
def atualizar_status_registro(
    ...
    with SessionLocal() as db:
        ...
        if actor_level == NivelAcesso.ENCARREGADO.value and registro.usuario_registrador_id != actor_user_id:
            raise PermissionError("Encarregado só pode atualizar seus próprios registros.")
        updated = Repository.registros.atualizar_status(db, registro_id, parsed, tenant_id=tenant_id)
        return {"ok": True, "registro": registro_to_dict_with_images(db, updated)}


def deletar_registro(tenant_id: int, actor_user_id: int, actor_level: str, registro_id: int) -> dict:
...
def anexar_imagem_registro(
    ...
    try:
        ...
        req = _req.Request(imagem_url, headers={"User-Agent": "ObraLog/1.0"})
        ...
        storage_path = upload_imagem_registro(
            tenant_id=tenant_id,
        ...
        logger.warning("Não foi possível armazenar imagem (registro %s): %s", registro_id, exc)
        return {"ok": False, "message": f"Não foi possível baixar ou armazenar a imagem: {exc}"}
